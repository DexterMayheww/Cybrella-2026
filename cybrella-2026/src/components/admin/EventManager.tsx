...
export default function EventManager() {
	...
	const handleDeleteCategory = async (id: string) => {
		if (!confirm("TERMINATE SECTOR? CONFIRM DELETION.")) return;
		try {
	...
	const deleteEvent = async (id: string) => {
		if (!confirm("DELETE PROTOCOL INITIATED. CONFIRM?")) return;
		try {
	...
	const handleReorderEvents = async (newCategoryOrder: EventData[], categoryName: string) => {
		const categoryStartIdx = events.findIndex(e => e.category === categoryName);
		const itemsWithNewOrder = newCategoryOrder.map((item, idx) => ({
			...item,
			order: (categoryStartIdx !== -1 ? categoryStartIdx : 0) + idx
		}));

		setEvents(prev =>
			prev.map(e => itemsWithNewOrder.find(item => item.id === e.id) || e)
				.sort((a, b) => a.order - b.order)
		);

		try {
			const batch = itemsWithNewOrder.map((event) =>
				updateDoc(doc(db, "events", event.id), { order: event.order })
			);
			await Promise.all(batch);
		} catch (e: any) {
			showNotify(`REORDER_SYNC_FAILED: ${e.message}`, "ERROR");
		}
	};
	...
	const handleReorderCategories = async (newOrder: Category[]) => {
		...
		try {
			const batch = newOrder.map((cat, index) =>
			...
			<div className="bg-white/5 border border-white/10 rounded-lg overflow-hidden backdrop-blur-md">
				...
				<div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					<div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<InputField label="URL_SLUG (AUTO-GENERATED)" value={form.slug} onChange={(v) => setForm({ ...form, slug: v })} icon={Globe} />
					</div>
