...
const EventCard = ({ event, index }: { event: EventCardProps; index: number }) => {
	return (
		<TiltWrapper>
			<Link href={`/events/${event.slug || event.id}`} className="block h-full w-full">
				<motion.div
					...
					<div className="relative h-full p-6 flex flex-col justify-between z-10 pointer-events-none">
						...
						<div className="flex justify-between items-start pointer-events-auto">
							...
							<div className="flex flex-col items-end">
								<span className="text-white font-black text-xl tracking-tighter leading-none">{event.date.split(" ")[1]}</span>
								<span className="text-xs text-gray-400 font-mono uppercase">{event.date.split(" ")[0]}</span>
							</div>
						</div>

						{/* Bottom Text */}
						<div className="pointer-events-auto">
							<h3 className="text-3xl font-black text-white mb-2 uppercase italic tracking-tighter transform translate-y-0 transition-transform duration-300 group-hover:-translate-y-2 drop-shadow-lg">
								{event.title}
							</h3>

							<div className="overflow-hidden">
								<p className="text-cyan-200/80 text-sm font-mono transform translate-y-full group-hover:translate-y-0 transition-transform duration-500 delay-100">
									CLICK_TO_REGISTER_
								</p>
							</div>
						</div>
